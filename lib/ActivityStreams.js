...
ActivityStreams.prototype = {
  ...
  _respondToPlacesRequests({msg, worker}) {
    ...
    switch (msg.type) {
      ...
      case am.type("HIGHLIGHTS_LINKS_REQUEST"):
        ...
        break;
      case am.type("REMOTE_TABS_LINKS_REQUEST"):
        provider.getRemoteTabsLinks(msg.data).then(links => {
          this._processAndSendLinks(links, "REMOTE_TABS_LINKS_RESPONSE", worker, msg.meta);
        });
        break;
  ...
  _get_memoized(cache) {
    ...
    return {
      ...
      getRemoteTabsLinks: cache.memoize("getRemoteTabsLinks", PlacesProvider.links.getRemoteTabsLinks.bind(linksObj)),
    ...
    if (simplePrefs.prefs["query.cache"]) {
      if (this._populatingCache && !this._populatingCache.places) {
        ...
        yield Promise.all([
            ...
            this._memoized.getRemoteTabsLinks(opt),
  ...
  _asyncBuildPreviewCache: Task.async(function*() {
    if (this._populatingCache && !this._populatingCache.preview) {
      ...
      promises.push(PlacesProvider.links.getRemoteTabsLinks().then(links => {
        placesLinks.push(...links);
      }));
